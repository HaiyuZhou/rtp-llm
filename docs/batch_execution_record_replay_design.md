...
实现跟踪：基础录制、请求事件及 trace 分析已接入；native 形状回放将在后续提交中加入，尚未完成全部生产验收。当前支持范围、配置和限制见 [使用说明](references/batch_recording.md)；下文保留完整设计目标。
...
session_id（一次录制会话）
  replica_id / dp_rank（独立调度域）
    scheduler_step_id（一次调度）
      execution_id（一次模型 forward）
        world_rank（该卡的实际执行和 kernel）
```

同一 TP 组由输入所有者产生执行 ID，并随输入传播。进程重启生成新的 session/实例身份。请求标识使用会话内稳定匿名 ID；sequence_id 区分同一请求展开出的多条序列。预留 parent_execution_id、model_role、substep_index。
...
| `request_id / sequence_id` | 请求和展开序列的稳定身份 |
| `phase` | prefill、decode；预留 draft、verify |
| `prompt_tokens` | 原始输入长度 |
| `q_tokens` | 本次 forward 对该序列实际计算的 token 数 |
| `kv_tokens_before` | 本次 forward 前已经存在的逻辑 KV token 数 |
| `prefix_cache_hit_tokens` | 初始命中前缀长度；来源确认后写入，未知为 null |
| `chunk_index` | chunked prefill 的分块序号，非 chunk 可为空 |
| `length_source` | host_input 或 device_snapshot |
| `is_fake` | 是否为同步/填充构造的序列 |

标准全注意力可用 `q_tokens + kv_tokens_before` 表示本次逻辑 attention 长度，但不能把它等同于物理 KV 容量或滑窗可见长度。
...
```json
{
  ...
  "model_role": "target",
  "requests": [
    {"batch_slot": 0, "request_id": "r17", "sequence_id": 0, "phase": "decode", "prompt_tokens": 1024, "q_tokens": 1, "kv_tokens_before": 2048},
    {"batch_slot": 1, "request_id": "r18", "sequence_id": 0, "phase": "prefill", "prompt_tokens": 4608, "q_tokens": 512, "kv_tokens_before": 4096}
  ]
}
```
...
具体接入函数在实现时核验。时间戳在状态转换处采集，不能使用 writer 写盘时间。first_token 是引擎输出发布时刻，不是 GPU 首次生成或客户端收包时刻；非流式请求可能到最终输出才产生此事件。多序列请求在首次发布任一序列的新 token 时记录一次，并携带 sequence_id；终态以整个请求为单位。

| 字段 | 定义 |
| --- | --- |
| `schema_version` | 第一版为 1 |
| `session_id / replica_id / dp_rank / request_id` | 请求关联键 |
| `owner_instance_id / clock_id` | owner 进程实例及单调时钟域，重启更换身份 |
...
| `scheduler_step_id / execution_id` | first_scheduled 必须关联首次执行；first_token 在可追溯时关联生产执行；其他未知为 null |
| `sequence_id` | 首 token 所属序列，请求级事件为 null |
...
```json
{"schema_version":1,"session_id":"s001","replica_id":"replica0","dp_rank":0,"request_id":"r17","owner_instance_id":"owner-01","clock_id":"owner-01-monotonic","event":"first_scheduled","event_seq":1,"timestamp_monotonic_ns":120010000000,"timestamp_unix_ns":1789344000010000000,"scheduler_step_id":9120,"execution_id":18231,"sequence_id":null,"output_mode":"streaming","reason":null,"error_code":null,"generated_tokens":null}
```
...
1. 调度/执行线程分配 execution_id，gather 时获得 slot、请求身份和实际输入长度。
2. CPU 输入在其有效期内复制为 recorder 自有数据；GPU 输入在确定的流依赖下冻结长度快照。
3. 执行身份随 TP 输入传播。forward 范围加入 execution_id，graph 路径补充实际执行元数据。
...
补充请求总耗时、首次排队、引擎首 token 延迟和终态占比，列出 partial 请求及事件丢失情况。通过请求键关联其参与的 execution_id；这些 batch 的 GPU 耗时属于共享执行，不能按请求数或 token 数均摊为请求独占耗时。

...
在启动各个引擎进程前设置相同的会话 ID。manifest 使用独立 owner 子目录，batch 文件由 alog.conf 决定：
...
`RTP_LLM_RECORD_DIR` 和 `RTP_LLM_RECORD_SESSION` 必须同时设置；启动后不支持动态修改。会话 ID 应在每次新实验时更换。`RTP_LLM_RECORD_FILE_BYTES` 已移除，设置该旧环境变量不再生效；batch 的轮转只由 alog.conf 的 max_file_size 管理。队列容量和总字节预算要求为正整数，无效值回退默认值。队列单位为记录条数，总字节预算按 owner 独立计算，包含提交给 batch logger 的数据。
...
batch worker 检查 INFO 级别后通过 `logBinaryMessage(INFO, json)` 写入配置名为 batch_schedule 的 logger；其他调用者可使用 `RTP_LLM_BATCH_SCHEDULE_LOG_INFO(json)`，使用不截断的原始消息接口，不加源码/时间前缀，不受 USE_CONSOLE_APPENDER/LOG_LEVEL 包装层重定向。路径、级别、格式、异步 flush 和轮转完全由 [alog.conf](../../rtp_llm/config/alog.conf) 的 batchScheduleAppender 控制，录制器不覆盖这些配置。初始化 alog 时可能创建空 batch 文件，即使录制尚未启用；没有 batch 配置则该专用 logger 默认不向 root 输出。录制器仅接受 `submitBatch(...)`，不创建动态 appender 或其他 JSONL 文件；owner 目录的 manifest 仍用临时文件 + rename 更新，已移除 `files` 和 `max_file_bytes` 字段。
...
交付语义为 **best effort**：缺日志就是缺失数据，不承诺无损。manifest 的 `submitted_to_alog` / `bytes_submitted` 表示提交量，不是实际落盘量；`written`、`bytes_written`、`sink_dropped` 为 null，`generated` 表示初始化完成、可接收记录时的提交尝试数（包含后续丢弃）；初始化前提交返回 false，不计入 generated；`dropped` / `errors` 仅覆盖录制器能观察的丢弃/错误。INFO 级别未启用时，batch 在序列化和预算检查前丢弃，计入 dropped，不计提交量或消耗预算。快照异常告警每分钟最多一次，TTFT slot 不匹配每个 batch 只计一次错误。`storage_backend=alog`、`delivery_policy=best_effort`，`complete` 始终 false（不能证明全量无损）。正常关闭 drain 上层队列并尝试 flush 本录制器的 alog 文件，`closed=true` 也不是无损保证。
...
- batch 层仅输出 `schema_version`、`owner_id`、`exec_id` 和 `requests`。`exec_id` 为十进制字符串，避免 JavaScript 等使用双精度数值的消费者丢失 int64 精度；消费者应保留字符串，不要转换为 Number。
...
- `sequence_id` 表示同一请求在本轮执行中展开出的序列位置索引，从 0 开始，每个请求独立编号；缺省为 0，仅非零时输出。它不保证跨轮稳定，尤其在 beam search 中，同一索引跨轮可能对应不同的 beam，不能用它追踪序列沿袭关系。
- `is_fake` 缺省为 false，仅为 true 时输出。`requests` 数组下标表示整个 batch 的 slot，与请求内的 `sequence_id` 不同。
...
`exec_id`、`owner_id`、`q_len`、`kv_len` 分别替代旧 key execution_id、owner_instance_id、q_tokens、kv_tokens_before；内部变量名不变，执行 ID 在内部仍为 int64，仅在 JSON 中转为字符串。q_len 和 kv_len 的单位均为 token，kv_len 始终表示本轮执行前已有 KV 的长度，不包含本轮 q_len。读取历史日志时可回退读取旧 key，并兼容旧版数字类型的执行 ID；解析旧数字时须保留整数精度，再规范化为字符串，已经舍入的 ID 无法恢复。
...
- 当前 manifest 提供模型/拓扑/cache 配置，报告比较配置差异；完整权重/软件指纹尚待补齐，不能把 config_match 解释为相同权重。普通引擎录制不再维护或传播 scheduler_step_id；离线回放没有在线调度 step。

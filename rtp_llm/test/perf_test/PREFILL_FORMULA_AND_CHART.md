...
New profiles do not need a `chart` section. Display names come from the top-level
`model_label` (falling back to `engine.model_type`); titles are generated by each
chart tool. Formula output uses `<model_label>_prefill_formula.txt`, key `PREFILL_TIME_FORMULA`,
and token unit 1024. The cold-chart annotation threshold defaults to 1048575.
Legacy profiles with a `chart` section remain readable for existing runs.
...
`prefill_formula_fit.py analyze-anomalies` detects measurement

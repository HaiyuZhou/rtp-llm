...
path = Path(
    "rtp_llm/test/perf_test/cache_grid/examples/"
...
```bash
python rtp_llm/test/perf_test/cache_grid/plot/generate_batch_interactive_chart.py \
...
坐标轴随 Batch 筛选自动切换，并在轴上及图外明确标注含义、单位：

- Batch 选“全部”：X=compute，Y=实际 cache，Z=batch 请求数。
- Batch 选具体大小：X=compute，Y=实际 cache，Z=整批 TTFT，不再保留恒定的 batch 轴。
- “长度统计”可选择每请求平均值或整批总量，只改变 cache/compute 的统计口径。
- 两种坐标模式的颜色始终表示整批 TTFT：耗时越长颜色越深。
  色阶范围固定为完整输入数据的 TTFT 范围，筛选前后相同颜色表示相同耗时。
...
```bash
node rtp_llm/test/perf_test/cache_grid/tests/generate_batch_interactive_chart_test.js \
...
```bash
--test_arg=--cache_shared_seed \
--test_arg=--cache_profile_runs=0
```

共享模式先执行一次长度为「待测 case 的最大 cache_len + cache_commit_tail_tokens」
的 seed，而不是强制发送 1M 输入；全量模板会自动选择能够覆盖最大缓存前缀的长度，
且不超过已有 case 的最大输入长度。所有 cold case 仍使用独立输入；cache-hit 请求
截取共享 seed 的前 C 个 token IDs，并拼接每轮独立的后缀。后缀在第 C 个 token 处
与 seed 分叉，每次请求仍严格验证 `reuse_len == cache_len`、输入长度和输出 1 token。
共享模式会在 runner 内调整待执行 case 的顺序：先执行 cache-hit，按 `cache_len` 从大到小；
相同 `cache_len` 按 `input_len` 从小到大；cold case 最后执行。默认独立 seed 模式仍严格
保持 JSON 声明顺序。续测只对剩余 case 重新排序，已完成 case 不会重复执行。

限制：只支持未分组的 batch=1、`dashsc_input_ids`、已知 cache block size，
不支持 HTTP prompt、预生成 case 文件或 cache profiler。不要保留 smoke 中的
`--cache_profile_runs=1` 和 `--cache_profile_case_ids` 参数。

该模式测量“共享长前缀已驻留”的请求耗时。每个 case 的 `elapsed_s` 不含共享 seed，
seed 构造和请求耗时保存在 `shared_seed_<id>.json` 及结果的 `shared_seed.elapsed_s`。
结果会标记 `seed_mode=shared_prefix_v1`，每个命中 case 记录 `shared_seed_id`；
不能与默认独立 seed 模式混用结果目录或直接混合做性能拟合。

服务重启续测时，会为剩余 case 重新执行一次共享 seed；历史 seed 记录单独保留。
如果实际命中小于目标，记录该失效请求，刷新共享 seed，再生成全新后缀重试一次。
刷新长度为「当前 case 的 cache_len + cache_commit_tail_tokens」，从原始共享 token IDs
截取，随 cache_len 递减而缩短；因为 case 已按 cache_len 降序执行，该长度也覆盖后续 case。
例如 cache_len=32768、commit tail=4096 时，只刷新 36864 tokens。刷新记录的
`refresh_input_len` 保存本次刷新长度。
失效请求不进入正式三轮统计；刷新记录在 `shared_seed_refresh_<id>.json`、结果及 case 的
`shared_seed_refreshes` 中，包含失效请求、seed 耗时。该恢复过程计入 case 的 `elapsed_s`。
重试仍不精确命中、命中超过目标或其他请求错误时，按原有失败策略终止，不无限重试。
因此大 grid 可能需要多次刷新，不能保证只 seed 一次。恢复时保持共享参数和其他配置不变。
